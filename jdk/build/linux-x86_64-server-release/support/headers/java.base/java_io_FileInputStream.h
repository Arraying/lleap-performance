/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class java_io_FileInputStream */

#ifndef _Included_java_io_FileInputStream
#define _Included_java_io_FileInputStream
#ifdef __cplusplus
extern "C" {
#endif
#undef java_io_FileInputStream_MAX_SKIP_BUFFER_SIZE
#define java_io_FileInputStream_MAX_SKIP_BUFFER_SIZE 2048L
#undef java_io_FileInputStream_DEFAULT_BUFFER_SIZE
#define java_io_FileInputStream_DEFAULT_BUFFER_SIZE 16384L
#undef java_io_FileInputStream_MAX_BUFFER_SIZE
#define java_io_FileInputStream_MAX_BUFFER_SIZE 2147483639L
#undef java_io_FileInputStream_DEFAULT_BUFFER_SIZE
#define java_io_FileInputStream_DEFAULT_BUFFER_SIZE 8192L
/*
 * Class:     java_io_FileInputStream
 * Method:    open0
 * Signature: (Ljava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_java_io_FileInputStream_open0
  (JNIEnv *, jobject, jstring);

/*
 * Class:     java_io_FileInputStream
 * Method:    read0
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_java_io_FileInputStream_read0
  (JNIEnv *, jobject);

/*
 * Class:     java_io_FileInputStream
 * Method:    readBytes
 * Signature: ([BII)I
 */
JNIEXPORT jint JNICALL Java_java_io_FileInputStream_readBytes
  (JNIEnv *, jobject, jbyteArray, jint, jint);

/*
 * Class:     java_io_FileInputStream
 * Method:    length0
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_java_io_FileInputStream_length0
  (JNIEnv *, jobject);

/*
 * Class:     java_io_FileInputStream
 * Method:    position0
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_java_io_FileInputStream_position0
  (JNIEnv *, jobject);

/*
 * Class:     java_io_FileInputStream
 * Method:    skip0
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_java_io_FileInputStream_skip0
  (JNIEnv *, jobject, jlong);

/*
 * Class:     java_io_FileInputStream
 * Method:    available0
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_java_io_FileInputStream_available0
  (JNIEnv *, jobject);

/*
 * Class:     java_io_FileInputStream
 * Method:    isRegularFile0
 * Signature: (Ljava/io/FileDescriptor;)Z
 */
JNIEXPORT jboolean JNICALL Java_java_io_FileInputStream_isRegularFile0
  (JNIEnv *, jobject, jobject);

/*
 * Class:     java_io_FileInputStream
 * Method:    initIDs
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_java_io_FileInputStream_initIDs
  (JNIEnv *, jclass);

#ifdef __cplusplus
}
#endif
#endif
